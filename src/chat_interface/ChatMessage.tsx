...
export function ChatMessage({ 
  ...
  if (type === 'app-buttons' && apps && apps.length > 0) {
    return (
      <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-4`}>
        <div className={`flex items-start gap-3 max-w-[280px] ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
          <Avatar className="w-8 h-8 flex-shrink-0">
            ...
            <div className={`px-4 py-3 rounded-2xl ${
              ...
              <div className={`grid ${apps.length <= 2 ? 'grid-cols-2' : 'grid-cols-2'} gap-2`}>
                {apps.slice(0, 4).map((app, index) => {
                  const IconComponent = appIcons[app.type];
                  const colorClass = appColors[app.type];
                  
                  return (
                    <Button
                      key={index}
                      onClick={() => onAppLaunch?.(app.type)}
                      className={`${colorClass} text-white border-0 rounded-xl p-3 h-auto flex flex-col items-center gap-1 transition-all duration-200 hover:scale-105 text-xs`}
                      size="sm"
                    >
                      <IconComponent />
